...
echo Open('form',array('name'=>'devoluciones_form','class'=>'form-horizontal','role'=>'form','novalidate'=>''));
    ...
    echo Open('div',array('class' => 'form-group col-sm-3'));
        echo Open('div',array('class' => 'input-group has-warning'));
            ...
            echo tagcontent('select',$option2,array('class'=>"form-control", 'ng-options'=>"bodega.id as bodega.nombre for bodega in bodegas", 'ng-model'=>"selectedBodega"));
        echo Close('div');
    echo Close('div');
    echo Open('div',array('class' => 'form-group col-sm-3'));
        echo Open('div',array('class' => 'input-group has-warning'));
...
echo Open('div',array('class'=>'col-md-12','id'=>'consulta','style'=>'font-size:16px','ng-show'=>'true'));
    ...
    echo 'LISTA DE PRECIOS POR GRUPOS<BR>';
    echo 'FECHAS: {{fecha_ini.value}} hasta  {{fecha_fin.value}}<BR>';
    echo 'INVENTARIO VALORADO : {{nombre_grupo}}<BR>';
