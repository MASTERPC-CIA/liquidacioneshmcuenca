    ...
    public function get_valores_liquid_por_aseguradora($fecha_desde, $fecha_hasta, $tipo_servicio, $tipo_pago, $tipo_comprobante) {
        ...
        if ($aseguradoras) {
            ...
            foreach ($aseguradoras as $key => $aseg) {
                ...
                $list_aseg[$cont_aseg] = (Object) array('aseg' => $aseg, 'lista_grupos' => $list_grupos, 'valor_aseg' => $total_grupos, 'val_iva_0_aseg' => $tot_grupos_iva_0, 'val_otro_iva_aseg' => $tot_grupos_otro_iva, 'val_iva_aseg' => $tot_grupos_iva);
                $cont_aseg++;
        ...
        $send['total_serv'] = $total_aseg;
        $send['nombre_serv'] = $this->get_name_servicio($tipo_servicio);
        $send['total_serv_iva'] = $tot_aseg_iva;
        $send['subtotal_serv_iva_0'] = $tot_aseg_iva_0;
        $send['subtotal_serv_otro_iva'] = $tot_aseg_otro_iva;
