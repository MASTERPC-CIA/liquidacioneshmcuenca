...
class Compras extends MX_Controller{
    ...
    public function index() {
        $res['top_nav_actions'] = $this->load->view('top_nav_actions_proveedor', '', TRUE);
        $res_slidebar['slidebar_actions'] = $this->load->view('slidebar', '', TRUE);
        $res['slidebar'] = $this->load->view('common/templates/slidebar_lte', $res_slidebar, TRUE);
        $res['title'] = 'Resumen Compras';
        $res['app'] = 'appLiquidacion';
        $res['angularjs'] = '';
        $res['css_angular'] = '';
        $this->load->view('common/templates/dashboard_lte_angularjs', $res);
    }
...
//            $compra['fact_data'] = $this->generic_model->get('billing_facturacompra', $where , $fields = '', $order_by = null, 
//                $rows_num = 0, $or_like_data = null, $and_like_data = null, $group_by = null, $or_where = null );
            ...
            if(get_settings('HM_CUENCA')==1){
                $compra['auxiliar_cont'] = $this->empleadocapacidad_model->get('aux_contabilidad');
                 $this->load->view('resumen_compras_hmc',$compra);
            }else{
                $this->load->view('resumen_compras',$compra); 
            }
           
//            echo tagcontent('script', "$('#fechacreaciond').val('')");
//            echo tagcontent('script', "$('#fechaemisiond').val('')");
//            echo tagcontent('script', "$('#fechaarchivadad').val('')");
//            echo tagcontent('script', "$('#fechacreacionh').val('')");
//            echo tagcontent('script', "$('#fechaemisionh').val('')");
//            echo tagcontent('script', "$('#fechaarchivadah').val('')");
